...
Client.prototype.send = function(request, callback) {
  ...
  var event = Event.stoppable(Event.preventable(new Event({
    name:     'before',
    request:  request,
    response: response,
    emitter:  this
  })));
  this.emit(event, function(err, event) {
    if (err) event.request.emit('error', err);
    ...
    if (event.isDefaultPrevented()) {
      ...
      req.on('response', function(res) {
        ...
        self.emit(event, function(err, event) {
          if (err) event.response.emit('error', err);
